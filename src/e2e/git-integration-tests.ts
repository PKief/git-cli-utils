...
export class GitIntegrationTests {
  ...
  async testGitBranchIntegration(): Promise<TestResult> {
    try {
      ...
      const result = await this.tester.runCommand('search-branches', ['\x03'], [
      ...
      return {
        passed: false,
        message: 'Git branch integration failed - not in git repo or git not available',
        duration: 0,
  ...
  async testGitCommitIntegration(): Promise<TestResult> {
    try {
      ...
      const result = await this.tester.runCommand('search-commits', ['\x03'], [
        'Search:', 'arrow keys', 'navigate'
      ]);
      ...
      return {
        passed: false,
        message: 'Git commit integration failed - no commits or git not available',
        duration: 0,
        error: error instanceof Error ? error : new Error(String(error))
      };
    }
  ...
  async testAliasCreation(): Promise<TestResult> {
    try {
      ...
      const { stdout } = await execAsync(`git config --global alias.${testAlias}`);
  ...
  async testListAliasesWithRealData(): Promise<TestResult> {
    const result = await this.tester.runCommand('list-aliases', [], [
      'git aliases'
    ]);
    
    return {
      ...result,
      message: result.passed ? 'List aliases works with real git config' : result.message
    };
  }
}